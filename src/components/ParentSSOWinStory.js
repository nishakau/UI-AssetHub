...
// alert("test");
    ...
    //    alert(statusGov[1]);
        ...
        var url = window.location.href;
        var ID=url.split('?');
        global.IDWinstory=ID[1];
        var resparam=  {
            winstoryId:global.IDWinstory,
            userId:sessionStorage.getItem('user_email'),
            "user_email":sessionStorage.getItem("user_email")
          }
        axios.post(global.Ip + global.Port + '/winstory/winstorySocialData/',resparam,{
            headers:resparam
        }) 
        .then(res => {
          var likedBy="";
          ...
          var likename;
            for(var i=0;i<=res.data.LIKES.length-1;i++){
               ...
               likedBy += '<div class="userLikeIcon  likepane " title='+likename+'><label>'+firsttwoletterLike+'</label></div>' ;
            //    if(res.data.USER_LIKE.length==1){
            //          IdforLikeunlike=res.data.USER_LIKE[0].LIKE_ID;
            //         var likeImg='../img/LikeFilled.jpg';
            //     }else{
            //         var likeImg='../img/like2.jpg';             
            //    }
            ...
            for(var j=0;j<=res.data.COMMENTS.length-1;j++){
                ...
                // var firsttwoletter = commentname.substring(0,1);
            ...
            if(res.data.LIKES.length<1){
                var LikeCount='Be first to like Win Story';
            }else{
                var LikeCount= res.data.LIKES.length;
            }

            if(res.data.COMMENTS.length<1){
                var Commentcnt='Be first to Comment on Win Story';
            }else{
      ...
      handleCommenteMangement = (e) => {
          var comment=document.getElementById('commmentbox').value;
                                ...
                                axios.post(global.Ip + global.Port + '/winstory/uploadComment', resparam,{ headers: {
                                    "user_email": sessionStorage.getItem("user_email")             
...
// console.log(likeNamesplit);
                                    ...
                                    likedBy += '<div class="loginname likepane col-md-1"  title='+likename+'><label>'+firsttwoletterLike+'</label></div>' ;
                                    if(res.data.USER_LIKE.length==1){
                                        IdforLikeunlike=res.data.USER_LIKE[0].LIKE_ID;
                                            var likeImg='../img/like_orange.png';
                                        }else{
                ...
                axios.post(global.Ip + global.Port + '/winstory/uploadLike', resparam,{ headers: {
                    "user_email": sessionStorage.getItem("user_email")             
...
// console.log(likeNamesplit);
                 ...
                 likedBy += '<span class="loginname likepane " title='+likename+'><label>'+firsttwoletterLike+'</label></div>' ;
